...
public class BufferList extends JPanel implements EBComponent
{
	...
	public BufferList(View view, final String position)
	{
		...
		tree.addTreeSelectionListener(new TreeSelectionListener() {
			public void valueChanged(TreeSelectionEvent e)
			{
				...
				BufferList.this.view.goToBuffer(buffer);
			}
		});
		createModel();
	...
	private void removeObsoleteDirNodes(BufferListTreeNode node)
	{
		...
		while (children.hasMoreElements())
		{
			BufferListTreeNode child = (BufferListTreeNode)children
					.nextElement();
	...
	private void saveExpansionState()
	{
		...
		Enumeration e = tree.getExpandedDescendants(new TreePath(tree
				.getModel().getRoot()));
		if (e != null)
		{
			while (e.hasMoreElements())
		...
		MiscUtilities.quicksort(buffers, new MiscUtilities.Compare() {
			public int compare(Object obj1, Object obj2)
			{
				...
				} else
				{
					...
					int cmpDir = MiscUtilities.compareStrings(dir1, dir2,
							sortIgnoreCase);
					if (cmpDir == 0)
					{
						return MiscUtilities.compareStrings(buf1.getName(),
			...
			if (paths != null)
			{
				for (int i = 0; i < paths.length; ++i)
				{
					...
					if (obj != null && obj instanceof String)
					{
						...
						while (children.hasMoreElements())
						{
							BufferListTreeNode childNode = (BufferListTreeNode)children
									.nextElement();
